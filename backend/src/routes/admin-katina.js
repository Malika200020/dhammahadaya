...
router.delete('/:year', async (req, res, next) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM gallery_images WHERE gallery = 'katina' AND gallery_key = $1;`, [req.params.year]);
    const result = await client.query('DELETE FROM katina_year WHERE year = $1 RETURNING year;', [req.params.year]);
    if (!result.rows[0]) {
