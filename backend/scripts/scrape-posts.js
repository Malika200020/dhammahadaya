// One-off migration script (build-spec §5.2 gap) — the "Posts" archive was
// never part of the original migration export (only the 5 Ninja Tables
// were, same gap as Ape Budu Hamuduruwo / Important Articles). Live
// /post/ is the full, unfiltered category-dhamma blog archive: 640 posts
// across 64 pages of an Elementor posts widget (query param
// `e-page-b9c945e`), each with its own real slug.
//
// Note: /post/'s newest ~150 entries are the exact same underlying posts
// already migrated separately as 'important_article' (verified: page 1 of
// /post/ is byte-identical, in the same order, to page 1 of
// /important-articles/). That's a property of the live site itself — the
// two live pages both draw from the same "category-dhamma" posts, one
// showing all of them, the other capped to the newest ~150 — not a
// scraping bug, so the resulting content overlap between the
// 'important_article' and 'newsletter' entry types here is intentional and
// mirrors production.
//
// This scrapes directly via raw HTTP + HTML parsing (cheerio), deliberately
// NOT through any AI/summarization step, to guarantee the Sinhala text is
// preserved byte-for-byte. Writes to a JSON file for manual spot-checking;
// a separate script (import-posts.js) does the actual DB insert after
// review.
//
// Usage: node scripts/scrape-posts.js
const fs = require('fs');
const path = require('path');
const cheerio = require('cheerio');

const LISTING_BASE = 'https://dhammahadaya.net/post/';
const LISTING_PARAM = 'e-page-b9c945e';
const DELAY_MS = 300;
const OUT_PATH = path.join(__dirname, 'posts-scraped.json');
...
// page 64 has 10 items, page 65 has 0).
async function collectListingUrls() {
  ...
  for (let page = 1; ; page++) {
    ...
    if (page % 10 === 0 || items.length < 10) console.log(`[listing page ${page}] ${items.length} items (${urls.length} total so far)`);
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      if (result.error) {
        ...
        results.push(result);
        if (n % 50 === 0 || n === urls.length) console.log(`[${n}/${urls.length}] ok (${results.length} ok, ${failures.length} failed so far)`);
      }
    } catch (err) {
